...
inline Patch const *emscripten::val::as<Patch const *>(void) const {
  ...
  emscripten::internal::EM_GENERIC_WIRE_TYPE result = emscripten::internal::_emval_as(
      handle, emscripten::internal::TypeID<emscripten::internal::AllowedRawPointer<Patch const>>::get(),
    &destructors
  );
  emscripten::internal::DestructorsRunner destructors_runner(destructors);

  return emscripten::internal::fromGenericWireType<Patch *>(result);
}
